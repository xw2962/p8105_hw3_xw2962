...
For making the two-panel plot, we first filtered out the rows for which state is NY and then filtered out the rows for which years are 2006 and 2010. Then we set the aesthetics using response as x variable and data_value as y variable, creating boxplots using geom_boxplot. And next we created two panels seperately for year 2006 and year 2010 using facet function. Last, we flipped the plot using coord_flip() and added some labels to make a better view.

From this plot, we can see that distributions for very good and good groups are the highest between 30 to 40 in both years. Distributions for excellent group and fair are between 10 to 30 in both years. And distributions for poor are lowest between 0 and 10 in both years.    
